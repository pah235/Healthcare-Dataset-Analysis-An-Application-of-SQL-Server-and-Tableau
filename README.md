...
# CAPSTONE PROJECT
# Healthcare Dataset Analysis: An Application of SQL Server and Tableau


<p align="justify"> <strong>Summary:</strong> The goal of this project is to apply data querying methods using SQL Server and data visualization with Tableau in healthcare data analysis. The project emphasizes the collection, cleaning, validation, and thorough analysis of data and the creation of detailed reports to provide insights for stakeholders’ decision-making.<br>
...
This project focuses on analyzing healthcare data from 55,500 anonymized patient records from 10 major hospitals across the United States. This fictional dataset provides an overview of hospital admissions, medical conditions, medications, insurance providers, and treatment costs.

The goal of the project is to gain a better understanding of patient demographics (age, gender, and blood type) to identify which groups are most frequently hospitalized. It also aims to determine the most common conditions within specific patient groups, analyze treatment costs, lengths of stay, and types of admissions, and track medication and insurance outcomes. The results of this analysis will support stakeholders (such as data analysts, healthcare strategists, hospital administrators, and public health researchers) in making strategic decisions to optimize healthcare service delivery.


### 1.2 Business Task
**Scenario:** You are a data analyst at a national healthcare monitoring organization. You have been granted access to a dataset of 55,500 individual patient records from 10 major hospitals across the United States. This dataset captures an overview of hospital admissions, medical conditions, medications, insurance providers, and treatment costs. Your goal is to tell the story of patient care at these hospitals, identify trends, and help stakeholders make data-driven decisions.

The key questions to answer in the analysis, as requested by stakeholders (questions developed from the dataset documentation), include:
...
This project follows the steps of the data analysis process from the Google Data Analytics Professional Certificate, which include ***ask, prepare, process, analyze, share, and act.*** These steps are presented in the five main parts of the project:
1. **Introduction:** Ask
...
In this project, I use **descriptive analytics** as the primary approach to explore and understand the overall characteristics of the healthcare dataset. 

Since the dataset is synthetic and does not represent real-world medical or operational dynamics, applying predictive or diagnostic analytics would likely generate patterns that are not clinically or operationally meaningful. In addition, the role of this project is to demonstrate the analytical capabilities of a **data analyst**, focusing on data preparation, exploration, and visualization — rather than providing medical interpretation or domain-specific modeling. Concentrating on descriptive analysis ensures a thorough, transparent, and technically sound exploration of the data within the scope of this capstone project.

The goal of this approach is to describe, summarize, and visualize the key features of the hospital data. This helps stakeholders understand who the primary patients are, which conditions are most common, and the length of hospital stays and treatment outcomes across different healthcare facilities. From there, it provides a foundation for more advanced analyses later on, such as diagnostic or predictive analytics.

#### *2.1.3 Tools and Software*

The main tools used in the project include:
- **SQL Server (MS SQL):** Techniques used include connecting to the database via Azure Data Studio; basic queries (`WHERE`, logical operators, arithmetic operators); data aggregation with `GROUP BY`, `HAVING`, `CASE WHEN`, and numeric, string, and date functions; querying data from multiple tables with `JOIN`; and working with subqueries, Common Table Expressions (`CTEs`), and views to optimize queries.
...
This dataset provides detailed insight into hospital operations and patient demographics at major hospitals in the United States. It includes records of admissions, diagnoses, treatments, and associated costs, offering a means to explore insights that can enhance healthcare decision-making, optimize costs, and improve patient outcomes.
...
The dataset and business questions are taken from the Onyx Data *DataDNA April 2025 Dataset Challenge*, available [here]([https://](https://onyxdata.kit.com/datadna-apr-2025)).
...
In this step, I listed the table’s columns and data attributes, especially those that are required, to identify which data fields need to be checked. The SQL query below retrieves the schema of the `healthcare_analysis` table:
...
In this section, I assess the completeness of the data and identify missing values in each column. The query uses the aggregate `COUNT()` function with `CASE WHEN ... THEN `clauses to count `NULL` values in each field, and compares the result with the original total record count to ensure no data was lost during import. This step helps confirm the integrity and reliability of the dataset before deeper analysis.
...
***Result:*** The dataset has 55,500 rows.
Comparing the total number of records in the dataset with the count from the original source data shows that no data was lost during extraction.
...
***Result:*** The query shows that there are no `NULL` values in any column.

### 3.4 Remove Duplicate or Irrelevant Observations**
The goal of this section is to remove duplicate records to ensure each patient appears only once. The main technique involves using `GROUP BY` ... `HAVING COUNT(*) > 1` to detect duplicates and `ROW_NUMBER() OVER (PARTITION BY ...)` to identify redundant records to delete. Removing duplicate data improves the accuracy of statistics and subsequent analyses.
...
This section focuses on normalizing the data by validating categorical values such as gender, blood type, admission type, and insurance provider. Queries use `SELECT DISTINCT` to detect formatting errors and logical conditions to check for invalid values (such as negative age or discharge dates earlier than admission dates). This ensures the data is consistent and accurately reflects reality.
``` sql
...
***Result*:** All columns have appropriate data structures.
...
***Result*:** All age and date values are valid.

### 3.6 Handling Outliers
This step aims to identify and handle outliers in the quantitative variable `billing_amount`, ensuring that values are not skewed. SQL is used to check for negative values and to apply statistical functions (`AVG()` and `STDEV()`) to find values exceeding three standard deviations from the mean. Removing these outliers helps improve the quality and reliability of cost analysis.
``` sql
...
***Result:*** 108 records have a negative `billing_amount`.
...
***Solution:*** Remove the records with negative values, as they represent a very small proportion of the total sample.
``` sql
DELETE FROM healthcare_analysis
WHERE billing_amount < 0;
```

***Result:*** 108 records with negative cost have been removed.

After this step, all `billing_amount` values are positive and valid. We then check for any values exceeding three standard deviations from the mean:
...
After cleaning, this section helps verify the overall validity of the data by confirming that there are no missing values, duplicates, or logical errors. Descriptive statistics are summarized to ensure that the dataset accurately reflects reality: the number of medical records, the categories of diseases, hospitals, insurance providers, and prescribed medications. This is the final confirmation step before data exploration.
After the data cleaning process, the dataset is logically consistent, adheres to required fields, and is ready for in-depth analysis. The data shows: `55,392` unique patient records for the period `2019–2024`; `6` primary diagnosed medical conditions; `10` hospitals; `4` insurance companies covering patients; and `5` commonly prescribed medications.

### 3.9 Create Analytical View
This section builds an aggregated view v_healthcare_analysis to prepare for analysis and visualization queries. The `CREATE OR ALTER VIEW` statement uses date functions (`YEAR`, `EOMONTH`, `DATEPART`), `CASE WHEN`, and `DATEDIFF` to add calculated fields such as admission year, month-end, weekday and weekday name, weekend/weekday flag, length of stay, cost per day, and age group. This view helps standardize the data and improve efficiency for further analysis.
...
After preparing the dataset for analysis and performing basic analysis using SQL, I used the cleaned data view `(v_healthcare_analysis)` to perform advanced data manipulation for filtering, sorting, and aggregate calculations. Then, Tableau was used to create charts that visualize trends and capture detailed insights.

### 4.1 Analysis of Patient Demographics
The SQL queries use `GROUP BY` to aggregate by age group, gender, and blood type; use `COUNT`, `CAST`, and division to calculate percentages; `ORDER BY` to rank groups; subqueries in `SELECT` to obtain the total number of patients; and a `CTE` with `ROW_NUMBER()` to identify the top patient groups by admission year.
...
>- *Age distribution:* The **18–44** age group is the largest, with 21,821 patients (39.32%), followed by **45–64** (30%). The older age groups (65+) make up about 30% of admissions, while the under-18 group is almost negligible (~0.3%). Thus, most patients are working-age adults (**18–64** years old).
>- *Gender distribution:* **Females** are the majority with 27,750 patients (50%), about 10 percentage points higher than males. Non-binary individuals account for 10%, reflecting gender diversity in the dataset. Overall, females are the most common hospitalized group.
>- *Blood type distribution:* **A+** is the most common blood type with 19,425 patients (35%), followed by O+ (25%) and O– (15%). The remaining blood types are relatively rare (~5% each). Thus, **A+ and O+** dominate the patient population.
...
> ***Initial observations:*** 
> - The group of **18–44** year old females with blood type A+ is the most common (7.83% of patients). Overall, **female patients with blood type A+** in the **18–44** and **45–64** age ranges are the most frequently hospitalized groups. 
> - From **2019 to 2024**, these two groups consistently rank in the top in terms of patient admissions. Males in the same age ranges have lower admission rates but still occupy high positions in the top 10. Therefore, **younger and middle-aged female patients** (especially those with blood type A+) are the most frequently admitted group.

### 4.2 Analysis of Common Medical Conditions and Distribution by Demographics
The SQL queries use `GROUP BY`, `COUNT`, and `SUM` to tally conditions; `CASE WHEN` to calculate rates by demographic features; a `CTE` to break calculation steps; and joins to find the group with the highest count.
...
> ***Initial observations:*** 
>- The combined distribution by age, gender, and blood type shows that for every condition, the **18–44** age group has the highest patient count. 
>- **Female** patients with blood type **A+** dominate many conditions (Hypertension, Asthma, Cancer). 
>- **Male** patients with blood type **O+** dominate in **Diabetes**.

### 4.3 Analysis of Hospital Stay by Condition, Hospital, and Admission Type
The SQL queries use window functions (`AVG` ... `OVER`, `PERCENTILE_CONT`) to compute averages and percentiles; `GROUP BY`to compare by hospital or admission type; `ORDER BY` to find the groups with the longest stays; and arithmetic to normalize results.
...
> ***Initial observation*:** 
> - The average hospital length of stay is **fairly consistent** across conditions. 
> - Patients typically stay around **15.4 to 15.6 days** on average. No single condition shows a significantly longer stay, though **Hypertension and Arthritis** are slightly higher. 
> - The 90th percentile of length of stay indicates that the most severe 10% of cases extend over **27–28 days**.
...
>- **Mayo Clinic** tends to have higher-than-average stays (≈16.0 days) for most conditions. **Houston Methodist and Johns Hopkins** have the largest numbers of cases, with more stable stays (≈15.3–15.6 days). 
>- **Asthma** has slightly lower stays at some hospitals (≈14.6–16.3 days).
...
> ***Initial observation:***
>- *Regarding admission type*, **Emergency** admissions generally have the highest average stays (**15.5–15.8 days**), **elective** slightly lower (**15.3–15.7**), and **urgent** similar or slightly lower than elective (**15.3–15.5**). 
...
>- Thus, **emergency** admissions tend to stay **a bit longer** (0.2–0.3 days) than other types, but overall differences are small, indicating treatment durations are fairly consistent across admission types.

### 4.4 Analysis of Treatment Costs by Condition and Hospital
The SQL queries apply window functions (`AVG` ... `OVER`, `PERCENTILE_CONT`, `STDEV`, `MIN`, `MAX`) to describe costs; `GROUP BY` to compare across hospitals; `ORDER BY` to identify highest costs; and arithmetic to normalize the data.
...
> ***Initial observations:*** 
>- The average treatment cost for each condition is quite uniform, around **\$25,300–\$25,800**, with no large differences between condition groups. 
>- Although the mean and median are very close, the high standard deviation (**~$14,000**) indicates substantial variability among cases or between hospitals. 
...
> ***Initial observation:*** 
>- Looking at cost differences between hospitals, generally the costs across conditions and hospitals seem similar (**\$24,000–\$28,000**), but there are notable exceptions. Asthma has the greatest cost variation: **Mayo Clinic** has the **highest** asthma treatment cost at \$28,370, while **UCSF Medical Center** has the **lowest** at \$23,181 (a difference of over \$5,000). **Cancer** has the most uniform costs. Cancer costs vary the most across hospitals, ranging from **\$24,821 to \$25,659 by \$800.**
>- **High-Cost Hospitals:** Mayo Clinic tends to have the highest pricing overall, leading in costs for Asthma and Obesity. UCLA Medical Center is also in the high-cost group, topping costs for Arthritis. **Hospitals with Polarized Costs:** UCSF Medical Center is a clear example: it has the lowest costs for Asthma and Diabetes, but the highest for Hypertension. **Lower-Cost Hospitals:** NewYork-Presbyterian Hospital reports the lowest costs for Cancer and Obesity.

### 4.5. Analysis of patient count and treatment outcomes
The SQL queries use `GROUP BY` to aggregate by hospital; `CASE WHEN` + `SUM` to count cases by test result category; arithmetic operators to calculate percentages; and `ORDER BY` to identify the largest hospitals and compare treatment quality.
...
> ***Initial observation:***
>- Top hospitals by patient volume: **Houston Methodist Hospital** leads with 20,362 patients, nearly double **Johns Hopkins Hospital** (11,248) and nearly triple **UCLA Medical Center** (6,836). These three hospitals account for the largest share of patients in the system. Next is a **group of similarly sized hospitals**, each with roughly **6,900 to 7,500** patients.
>- Comparing **test results** (an indicator of treatment quality): Although Houston Methodist treats more than twice as many patients as Johns Hopkins and three times as many as UCLA, their distributions of test results are **nearly identical**. The percentage of **Normal** results is about **9.6–10.8%** across hospitals, almost the same for all. **Abnormal results** account for about **54–56%**, highest at Cedars-Sinai Medical Center (55.81%), Cleveland Clinic (55.48%), and Houston Methodist (55.08%). **A minor difference** is that **NewYork-Presbyterian Hospital** has the **highest Normal rate (10.76%)** and the lowest Inconclusive rate (33.95%), making it the hospital closest to the overall trend.

### 4.6. Analysis of most frequently prescribed medications by condition
The SQL queries use `GROUP BY` to aggregate by medication; the window function `PARTITION BY` to calculate proportions within each condition; a `CTE` (common table expression) to prepare intermediate data; `MIN`/`MAX` + arithmetic to measure differences in prescription proportions; and CASE WHEN to classify consistency levels.
...
> ***Initial observation:*** 
>- Regarding the most commonly prescribed medications for each condition: the distributions among these medications are **fairly** even for most conditions, with no single medication overwhelmingly dominant. Each condition has five top medications according to the dataset, and the percentage of patients using each is quite balanced, typically around **19–21%.**

![F11](Images/Figure-11.png)

> ***Initial observation:*** 
>- Regarding the **variation (spread)** in prescription percentages between hospitals for each condition: the results show that **Diabetes and Hypertension** are the most uniform (**“Highly consistent”**) – the spread between hospitals is only about **3–4%**. **Cancer and Obesity** are **fairly consistent** with spreads around **4–6%**. **Arthritis** is **moderately consistent** with spreads of **7–9%**. **Asthma** is the least consistent (**“Not consistent”**), especially for the drug **Penicillin** (spread up to **14.6%**).

### 4.7. Analysis of admission types
The SQL queries apply `GROUP BY` by medical condition and admission type; use `AVG` and `COUNT` to compute lengths of stay and costs; arithmetic to normalize cost per day; and `ORDER BY` to find the groups with the highest case counts and costs.
...
> ***Initial observation:***
>- Regarding the most common admission methods, in general patients are more often admitted via **Elective (planned) or Urgent** admissions **than Emergency**, and this distribution is quite uniform across admission types for most conditions. 
>- For chronic, common conditions such as **Diabetes, Hypertension, and Obesity**, patient admissions are distributed almost evenly across the three types. 
>- For less common conditions (**Arthritis, Asthma, Cancer**), patient counts are also very evenly distributed among the three admission types, with each type having between roughly **1,280 and 1,870 cases.**
...
> ***Initial observation:***
>- Admission type affects costs, with a **clear distinction** between average total cost and average cost per day. 
>- **Impact on average total cost:** Differences between admission types are **not large**, with overall averages around **\$25,000–\$26,000.** **Elective (planned)** admissions have the **highest total costs**. For most conditions, **Elective** cases have the **highest** average total cost. 
>- **Impact on average cost per day:** **Urgent** admissions have the **highest** per-day costs. For most conditions (except Asthma and Obesity), **Urgent** admissions have the highest average cost per day. **Asthma** is an exception: it has the highest cost per day under **Elective** admissions (notably **$3,648 per day** for Asthma under Elective).

### 4.8. Analysis of insurance providers
The SQL queries use `GROUP BY` by insurance provider; `AVG`, `MIN`, `MAX`, `COUNT` to describe costs; a `CTE` to normalize data; `CASE WHEN` to calculate test result percentages; and `ORDER BY` to rank by size.
...
> ***Initial observation:*** 
>- **Medicare** has the largest number of patients (**50%**), the highest share in the dataset. It is followed by **UnitedHealthCare** (30%), **Aetna** (10%), and **Cigna** (10%). This indicates **Medicare** is the clearly **dominant** insurer in this data.
>- **Average treatment costs** vary only slightly across insurers, around **\$25,400–\$25,800**. **Cigna** has the **highest** average cost (\$25,812) and **Aetna** the **lowest** (\$25,460). However, note that billing_amount reflects total treatment cost and does not distinguish between the portion paid by insurance and the portion paid by patients. Therefore, cost differences between insurers only reflect the profile of the patients they cover, not the actual payments made by each insurer.
>- **Average length of stay** is relatively stable (**about 15.4–15.6 days**), with no significant differences among insurers. The rate of **abnormal** test results is **highest** for **Medicare (59.98%)**, while the other insurers are **around 50%**.

### 4.9. Analysis of geographic distribution (hospital locations)
The SQL queries use `GROUP BY` by hospital name and coordinates; `COUNT` and `SUM` to aggregate patient volumes and billing; and arithmetic functions with `ROUND` to prepare data for spatial analysis.
...
> ***Initial observation:*** 
>
> The map above shows the 10 largest hospitals in the U.S., distributed across the **Northeast, Midwest, South, and West** regions. The regional distribution is as follows:
>- **Northeast:** The Northeast region hosts many large hospitals such as Johns Hopkins Hospital, ***NewYork-Presbyterian Hospital, and Massachusetts General Hospital***. This region has a high population density and correspondingly records the largest total treatment costs and patient volumes.
>- **Midwest:** The Midwest includes ***Mayo Clinic, Cleveland Clinic, and Northwestern Memorial Hospital.*** These hospitals are relatively close together, serving a concentrated population but on a smaller scale than the Northeast centers.
>- **Southern U.S.:** The Southern U.S., highlighted by ***Houston Methodist Hospital***, shows the largest treatment scale on the map. Houston has the highest total billing (treatment costs) and also the highest patient count among the group (61,086 patients).
>- **Western U.S.:** The West includes ***UCLA Medical Center, Cedars-Sinai Medical Center, and UCSF Medical Center*** in California. This is a densely populated region with relatively high total treatment costs, but on a smaller scale than Houston or Johns Hopkins.
>
> **Size differences between regions:** Hospitals in **Texas** and the **Northeast** have significantly **higher** total treatment costs. The **West** and **Midwest** have many hospitals, but their combined costs and patient volumes tend to be **lower**.
>
> **Density and distances:** Hospitals in the **Northeast** and **Midwest** are geographically **close**, creating a high density of healthcare facilities. In contrast, the **West** and **South** have **greater** distances between major medical centers, resulting in a wider service area per hospital. The large scale in these regions (especially **Houston Methodist Hospital**) reflects their broader coverage areas and lower hospital density.

## 5. Key Insights

>•	***Overall conclusions on patient demographics:*** The most common patient group is adults aged **18–44** (39%), followed by those aged **45–64** (30%). **Females** account for the highest share of patients (50%), about 10 percentage points more than males. By blood type, **A+** (35%) and **O+** (25%) are the most prevalent. Considering all three factors, **young women (18–44) with blood type A+** form the single **largest** group (nearly 8% of cases). In general, certain patient groups — especially **younger women with blood type A+ or O+** — have significantly **higher** hospitalization rates than others, reflecting demographic and physiological influences on disease prevalence and healthcare needs.

>•	***Overall conclusions on medical conditions:*** The most common conditions are **Hypertension** (25%), **Diabetes** (25%), and **Obesity** (23%). The primary affected age groups are **18–44** (~39%) and **45–64** (~30%), and **females** are the most affected (notably for Asthma, Arthritis, and Cancer). The most common blood types are **A+** (most prevalent in Asthma, Hypertension, and Cancer) and **O+** (most prevalent in Diabetes). The highest overall risk group is **females aged 18–44 with blood type A+**. Notably, for Diabetes specifically, the highest-risk group is **males aged 18–44 with blood type O+.**

>•	***Overall conclusions on length of stay:*** The **average length of stay** is about **15–16 days** for almost all conditions. **Hypertension** has the **highest** average LOS (15.63 days) and Asthma the lowest (15.39 days). However, this duration varies only slightly by admission type and by hospital (differences are on the order of **±1.5 days** between hospitals). **Emergency** admissions have slightly longer stays on average than Elective or Urgent admissions. Overall, length of stay is fairly **uniform** across conditions, hospitals, and admission types, reflecting standardized care processes.

>•	***Overall conclusions on treatment costs:*** **Average treatment costs** for the common conditions are around **\$25,500**. However, there are **differences** between hospitals, especially for **Asthma**. Some hospitals have costs about **\$1,000–\$2,000** **higher** than the overall average.

>•	**Overall conclusions on patient volumes and outcomes:** **Large** hospitals like **Houston Methodist, Johns Hopkins, and UCLA Medical Center** treat the most patients and also have similar distributions of test results. **NewYork-Presbyterian** stands out with the **highest** rate of **“Normal”** test results.

>•	***Overall conclusions on admission types:*** **No single admission** type dominates; patient admissions are split roughly equally among Emergency, Urgent, and Elective across the conditions. In most cases, the number of cases is nearly the same for each type, although for chronic conditions (**Hypertension, Diabetes, Obesity**), **Elective and Urgent** admissions are **slightly** more common than **Emergency**. Admission type has the greatest impact on treatment costs, especially **per-day** costs: **Elective and Urgent** admissions tend to be a bit more **expensive**.

>•	***Overall conclusions on insurance providers:*** Although **Medicare** has the **largest** patient count and the highest abnormal result rate, the average costs and lengths of stay are quite similar across insurers. Since the data only shows total billing (not how much insurers actually paid), we cannot conclude which insurer “pays more” or is more “efficient”; we can only say that the average cost recorded per patient is about the same for each insurance company.

>•	***Overall conclusions on geographic distribution:*** The distribution of top hospitals is uneven nationwide. Medical centers in densely populated regions (**Northeast, South, and West**) have **higher** patient counts and total treatment costs, whereas the **Midwest** has a high density of hospitals but a smaller scale of patients and costs per hospital.
